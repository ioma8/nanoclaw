...
export function createIpcTools(ctx: IpcMcpContext): Tool[] {
...
• once: Local time WITHOUT "Z" suffix (e.g., "2026-02-01T15:30:00"). Do NOT use UTC/Z suffix.`,
      parameters: z.object({
        ...
        target_group: z.string().default('').describe('Target group folder (main only). Use empty string to default to current group.')
      }),
      execute: async (args) => {
        ...
        if (args.schedule_type === 'cron') {
          ...
          if (isNaN(date.getTime())) {
            return `Invalid timestamp: "${args.schedule_value}". Use ISO 8601 format like "2026-02-01T15:30:00" (no Z suffix).`;
          }
        }

        // Non-main groups can only schedule for themselves
        const targetGroup = isMain && args.target_group.trim() ? args.target_group.trim() : groupFolder;
